...
enum
{
    ...
    NPC_WYRMREST_SKYTALON           = 32535, //30161, // dragon vehicle in 3-d Phase
...
struct MANGOS_DLL_DECL boss_malygosAI : public ScriptedAI
{
    ...
    void SpellHitTarget(Unit* pUnit, const SpellEntry* pSpell)
    {
        ...
        {
            ((Creature*)pUnit)->SetInCombatWithZone();
    ...
    void JustSummoned(Creature* pSummoned)
    {
        ...
        if (uiEntry == NPC_VORTEX || uiEntry == NPC_STATIC_FIELD || uiEntry == NPC_SURGE_OF_POWER)
    ...
    {
        if (Creature* pDisk = m_creature->getVictim()->SummonCreature(NPC_HOVER_DISK, pSummoned->GetPositionX(), pSummoned->GetPositionY(), FLOOR_Z+1.5f, 0, TEMPSUMMON_CORPSE_DESPAWN, 0))
        {
            pDisk->setFaction(35);
        ...
        if (uiEntry == NPC_NEXUS_LORD || uiEntry == NPC_SCION_OF_ETERNITY)
        {
            if (Creature* pDisk = GetClosestCreatureWithEntry(pSummoned, NPC_HOVER_DISK, 3.0f))
            {
                pDisk->setFaction(35);
                pDisk->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                pDisk->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                pDisk->CastSpell(pDisk, SPELL_FLIGHT, true);
            }
        }
    }
    ...
    void MovementInform(uint32 uiType, uint32 uiPointId)
    {
        ...
        else if (uiPointId == POINT_ID_DEEP_BREATH)
        {
            ...
            if (Creature* pTemp = m_creature->SummonCreature(NPC_SURGE_OF_POWER, CENTER_X, CENTER_Y, FLOOR_Z, 0, TEMPSUMMON_TIMED_DESPAWN, 15000))
            {
                pTemp->setFaction(14);
                pTemp->CastSpell(m_creature, SPELL_SURGE_OF_POWER_BREATH, true);
            }
        ...
        else if (m_uiPhase == PHASE_ADDS)
        {
            ...
            if (m_uiCheckTimer <= uiDiff)
            {
                if (!IsThereAnyAdd())
                {
                    ...
                    if (Creature* pTempTarget = m_creature->SummonCreature(NPC_VORTEX, CENTER_X, CENTER_Y, FLOOR_Z, 0.0f, TEMPSUMMON_TIMED_DESPAWN, 10000))
                    {
                        pTempTarget->SetFloatValue(OBJECT_FIELD_SCALE_X, 1.5f);
                        pTempTarget->CastSpell(pTempTarget, SPELL_DESTROY_PLATFORM_PRE, false);
                        pTempTarget->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                    }

                    // workaround for mounting on dragons - players have to click quickly in order not to fall...
                    Map *pMap = m_creature->GetMap();
                    if (pMap)
                    {
                        Map::PlayerList const &lPlayers = pMap->GetPlayers();
                ...
                if (pMap)
                {
                    ...
                    for (Map::PlayerList::const_iterator itr = lPlayers.begin(); itr != lPlayers.end(); ++itr)
                    {
                        ...
                        // workaround - check Z position
                        if (pPlayer->GetPositionZ() > FLOOR_Z + 7.0f && m_uiPhase == PHASE_ADDS)
            ...
            {
                m_creature->GetMotionMaster()->Clear();
                m_creature->GetMotionMaster()->MovePoint(POINT_ID_DEEP_BREATH, CENTER_X, CENTER_Y, AIR_Z);
                m_bReadyForWPMove = false;
                m_uiTimer = 20000;
        ...
        else if (m_uiPhase == PHASE_DRAGONS)
        {
            if (m_uiSubPhase == SUBPHASE_DESTROY_PLATFORM_1)
            {
                if (m_uiTimer <= uiDiff)
                {
                    ...
                    if (Creature* pTempTarget = m_creature->SummonCreature(NPC_VORTEX, CENTER_X, CENTER_Y, FLOOR_Z, 0, TEMPSUMMON_TIMED_DESPAWN, 5000))
                        ...
                        if (!lPlayers.isEmpty())
                            for (Map::PlayerList::const_iterator itr = lPlayers.begin(); itr != lPlayers.end(); ++itr)
                                if (Player* pPlayer = itr->getSource())
                                {
                                    ...
                                    switch (urand(0, 2))
                                    {
                                        case 0: fX = 0.0f; break;
                                        case 1: fX = 15.0f; break;
                                        case 2: fX = -15.0f; break;
                                    }

                                    switch (urand(0, 2))
                                    {
                                        case 0: fY = 0.0f; break;
                                        case 1: fY = 15.0f; break;
                                        case 2: fY = -15.0f; break;
                                    }

                                    if (fX == 0.0f && fY == 0.0f)
                                        fX = 15.0f;

                                    if (Creature* pSkytalon = pPlayer->SummonCreature(NPC_WYRMREST_SKYTALON, pPlayer->GetPositionX()+fX, pPlayer->GetPositionY()+fY, FLOOR_Z-85.0f, 0, TEMPSUMMON_TIMED_DESPAWN, 3000))
                                    {
                                        pSkytalon->SetByteValue(UNIT_FIELD_BYTES_1, 3, UNIT_BYTE1_FLAG_ALWAYS_STAND | UNIT_BYTE1_FLAG_UNK_2);
                                        pSkytalon->CastSpell(pSkytalon, SPELL_FLIGHT, true);
                                        pSkytalon->GetMotionMaster()->MovePoint(0, pPlayer->GetPositionX(), pPlayer->GetPositionY(), FLOOR_Z-85.0f);
                                    }
                                }
                    }*/

                    m_creature->GetMotionMaster()->MovePoint(0, CENTER_X, CENTER_Y, FLOOR_Z + 80.0f);
                ...
                if (m_uiTimer<= uiDiff)
                {
                    ...
                    if (pMap)
                    {
                        ...
                        if (!lPlayers.isEmpty())
                            for (Map::PlayerList::const_iterator itr = lPlayers.begin(); itr != lPlayers.end(); ++itr)
                                if (Player* pPlayer = itr->getSource())
                                    if (Creature* pTemp = pPlayer->SummonCreature(NPC_WYRMREST_SKYTALON, pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), 0, TEMPSUMMON_CORPSE_DESPAWN, 0))
                                    {
                                        //if (pTemp->CreateVehicleKit(165))
                                        {
...
struct MANGOS_DLL_DECL npc_nexus_lordAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        m_creature->AddSplineFlag(SPLINEFLAG_FLYING);
        m_creature->GetMotionMaster()->MovePoint(1, urand(PLATFORM_MIN_X, PLATFORM_MAX_X), urand(PLATFORM_MIN_Y, PLATFORM_MAX_Y), FLOOR_Z);

        // creature should be mounted on Hover Disc
    }
...
struct MANGOS_DLL_DECL npc_scion_of_eternityAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        {
            m_creature->GetMotionMaster()->MovePoint(0, urand(PLATFORM_MIN_X, PLATFORM_MAX_X), urand(PLATFORM_MIN_Y, PLATFORM_MAX_Y), FLOOR_Z+10.0f+urand(0, 10));
            m_uiMoveTimer = 4000;
        }
...
## npc_arcane_overload
######*/
struct MANGOS_DLL_DECL npc_arcane_overloadAI : public ScriptedAI
{
    npc_arcane_overloadAI(Creature *pCreature) : ScriptedAI(pCreature){}
    void Reset(){SetCombatMovement(false);}
